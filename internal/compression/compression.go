...
// Package compression wraps [connect.Compressor] with the size-limit and
// error-coding behavior every transport needs. It is transport-agnostic: the
// HTTP and WebSocket transports share it.
package compression
...
// Pool applies one [connect.Compressor] to whole messages.
type Pool struct {
	compressor connect.Compressor
}

// NewPool returns a [Pool] for compressor, or nil when compressor is nil.
func NewPool(compressor connect.Compressor) *Pool {
	if compressor == nil {
		return nil
	}
	return &Pool{
...
func (c *Pool) Decompress(dst *bytes.Buffer, src *bytes.Buffer, readMaxBytes int64) *connect.Error {
...
func (c *Pool) Compress(dst *bytes.Buffer, src *bytes.Buffer) *connect.Error {
...
// ReadOnlyPools is a read-only interface to a map of named [Pool]s.
type ReadOnlyPools interface {
	Get(string) *Pool
...
// NewReadOnlyPools indexes compressors by name, in preference order. A
// repeated name is ignored.
func NewReadOnlyPools(compressors []connect.Compressor) ReadOnlyPools {
	nameToPool := make(map[string]*Pool, len(compressors))
	...
	for _, compressor := range compressors {
		...
		nameToPool[name] = NewPool(compressor)
...
type namedPools struct {
	nameToPool          map[string]*Pool
	commaSeparatedNames string
}

func (m *namedPools) Get(name string) *Pool {
...
func (m *namedPools) Contains(name string) bool {
	_, ok := m.nameToPool[name]
	return ok
}

func (m *namedPools) CommaSeparatedNames() string {
