...
type duplexHTTPCall struct {
	...
	onResponseReceive func(*http.Response)
	validateResponse  func(*http.Response) *connect.Error
...
func (d *duplexHTTPCall) Send(payload messagePayload) (int64, error) {
	if d.streamType&connect.StreamTypeClient == 0 {
...
func (d *duplexHTTPCall) SetValidateResponse(validate func(*http.Response) *connect.Error) {
	d.validateResponse = validate
}

// awaitResponse blocks until the response is ready or cancelled. It reports
// whether the response arrived. If false, the response state may still be
// written by makeRequest and must not be read.
func (d *duplexHTTPCall) awaitResponse() bool {
	if !d.requestSent.Load() {
		return false
	}
	response, _ := d.blockUntilResponseReady()
	return response != nil
...
func (d *duplexHTTPCall) makeRequest() {
	...
	d.response = response
	if d.onResponseReceive != nil {
		d.onResponseReceive(response)
	}
	...
	if (d.streamType&connect.StreamTypeBidi) == connect.StreamTypeBidi && response.ProtoMajor < 2 {
