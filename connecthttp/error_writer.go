...
// NewErrorWriter constructs an ErrorWriter. Options may be passed to configure
// the error writer behaviour to match the handlers.
...
func NewErrorWriter(options ...Option) *ErrorWriter {
	opts := defaultOptions()
...
func (w *ErrorWriter) writeConnectUnary(response http.ResponseWriter, err error) error {
	response.WriteHeader(connectCodeToHTTP(connect.CodeOf(err)))
...
func (w *ErrorWriter) writeConnectStreaming(response http.ResponseWriter, err error) error {
	...
	// MarshalEndStream returns *connect.Error: check return value to avoid typed nils.
...
func (w *ErrorWriter) writeGRPC(ctx context.Context, response http.ResponseWriter, err error) error {
	trailers := make(http.Header, 2) // need space for at least code & message
	grpcErrorToTrailer(ctx, trailers, w.protobuf, err)
...
func (w *ErrorWriter) writeGRPCWeb(ctx context.Context, response http.ResponseWriter, err error) error {
	// This is a trailers-only response. To match the behavior of Envoy and
	// protocol_grpc.go, put the trailers in the HTTP headers.
	grpcErrorToTrailer(ctx, response.Header(), w.protobuf, err)
	response.WriteHeader(http.StatusOK)
	return nil
}
